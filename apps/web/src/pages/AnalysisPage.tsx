...
async function runWithConcurrency<T>(items: T[], concurrency: number, worker: (item: T, index: number) => Promise<void>): Promise<void> {
  ...
  const results = await Promise.allSettled(Array.from({ length: Math.min(concurrency, items.length) }, () => runner()));
...
export function AnalysisPage() {
  ...
  const effectivePerFileMbQuota = typeof infrastructure?.maxUploadMb === 'number'
  ...
  const processingConcurrency = Math.max(1, Number(infrastructure?.recommendedConcurrency) || 5);
  ...
  const run = async () => {
    ...
    try {
      ...
      if (sourceMode === 'files') {
        const oversized = selected.find((file) => file.size > effectivePerFileMbQuota * 1024 * 1024);
        if (oversized) throw new Error(`${oversized.name} 超過線上版單檔 ${effectivePerFileMbQuota} MB 上限；其他檔案不受其大小累加影響。`);
        const itemProgress = selected.map(() => 0);
        await runWithConcurrency(selected, processingConcurrency, async (file, index) => {
          const form = new FormData();
          form.append('files', file, (file as File & { webkitRelativePath?: string }).webkitRelativePath || file.name);
          form.append('batchName', `個別分析：${file.name}`);
          const result = await uploadFiles(form, (value) => {
            itemProgress[index] = value;
            setProgress(Math.round(itemProgress.reduce((sum, current) => sum + current, 0) / selected.length * 0.75));
          });
          const details = result.rejected?.map((item: any) => `${item.name}：${item.error}`).join('；');
          if (!result.sourceFileIds?.length) throw new Error(details || `${file.name} 沒有建立可合併的來源檔案`);
          if (result.processingErrors?.length) throw new Error(`${file.name} 分析失敗：${result.processingErrors.map((item: any) => item.error).join('；')}`);
          sourceFileIds.push(...result.sourceFileIds);
          itemProgress[index] = 100;
        });
      } else {
        let completed = 0;
        await runWithConcurrency(googleSheets, processingConcurrency, async (sheet, index) => {
          const result = (await api.post('/files/google-sheets', { sheets: [sheet], batchName: `個別分析：${sheet.name ?? `Google Sheet ${index + 1}`}` })).data;
          const details = result.rejected?.map((item: any) => `${item.name}：${item.error}`).join('；');
          if (!result.sourceFileIds?.length) throw new Error(details || `Google Sheet ${index + 1} 沒有建立可合併的來源檔案`);
          if (result.processingErrors?.length) throw new Error(`Google Sheet ${index + 1} 分析失敗：${result.processingErrors.map((item: any) => item.error).join('；')}`);
          sourceFileIds.push(...result.sourceFileIds);
          completed += 1;
          setProgress(Math.round(completed / googleSheets.length * 75));
        });
      }
      setProgress(82);
      const combined = (await api.post('/files/combine-existing', { sourceFileIds: [...new Set(sourceFileIds)], batchName: batchName.trim() || undefined, projectId: projectId || undefined })).data;
      await createOutput(combined.job);
      setSelected([]);
      setGoogleLinks('');
      setBatchName('');
      ...
      <div className="grid lg:grid-cols-[1.2fr_.8fr]">
        ...
        <div className="p-6">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="rounded-xl bg-slate-50 p-3"><div className="flex items-center justify-between text-sm"><span className="font-semibold text-slate-600">已加入</span><span className="font-black text-ink">{sourceCount} / {Number.isFinite(effectiveFileQuota) ? effectiveFileQuota : '無產品配額'}</span></div>{onlineLimited && <p className="mt-1 text-[11px] leading-5 text-amber-700">一次選取整批後，系統以 {processingConcurrency} 路平行分析再集中合併：最多 {effectiveFileQuota} 份、每份 {effectivePerFileMbQuota} MB（不再累加成單一請求）、輸出新檔 {infrastructure?.maxOutputMb} MB。</p>}{sourceMode === 'files' && selected.length > 0 && <div className="mt-2 max-h-20 overflow-y-auto text-xs text-slate-500">{selected.slice(0, 8).map((file) => <p className="truncate py-0.5" key={`${file.name}-${file.size}`}>{file.name}</p>)}</div>}</div>
