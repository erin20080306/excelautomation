...
import {
  Activity, Boxes, Braces, ChevronRight, CircleHelp, ClipboardCheck, Database, FileClock, FileSpreadsheet,
...
export function Layout() {
  ...
  return <div className="min-h-screen bg-[#F6F9F8]">
    ...
    {mobileOpen && <div className="fixed inset-0 z-[60] md:hidden"><button className="absolute inset-0 bg-slate-950/40" onClick={() => setMobileOpen(false)} aria-label="關閉選單" /><div className="absolute inset-y-0 left-0 w-[84%] max-w-sm overflow-y-auto bg-white p-4 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div className="flex items-center gap-3"><div className="rounded-xl bg-brand-600 p-2 text-white"><FileSpreadsheet /></div><span className="font-black text-ink">ExcelMaster</span></div><button onClick={() => setMobileOpen(false)} className="p-2"><X /></button></div><Navigation mobile /><button className="mt-6 flex w-full items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-500" onClick={logout}><LogOut size={17} />安全登出</button></div></div>}
