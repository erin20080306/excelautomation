...
import {
  Activity, BadgeDollarSign, Boxes, Braces, ChevronRight, CircleHelp, ClipboardCheck, Database, Download, FileClock, FileSpreadsheet,
...
export function Layout() {
  ...
  const visibleNavItems = session?.user.platformRole === 'SUPERADMIN' ? [...navItems, { path: '/admin', label: '平台管理', icon: ShieldCheck }] : navItems;
  ...
  const Navigation = ({ mobile = false }: { mobile?: boolean }) => <nav className="space-y-1.5">{visibleNavItems.map(({ path, label, icon: Icon }) => <NavLink key={path} to={path} onClick={() => mobile && setMobileOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${isActive ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}><Icon size={18} /><span>{label}</span></NavLink>)}</nav>;
