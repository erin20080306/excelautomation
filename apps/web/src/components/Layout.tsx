...
export function Layout() {
  ...
  const welcomeKey = `excelmaster.welcome-v2.${session?.user.id ?? 'guest'}`;
  const [welcomeOpen, setWelcomeOpen] = useState(() => localStorage.getItem(welcomeKey) !== 'seen');
  ...
  const closeWelcome = () => { localStorage.setItem(welcomeKey, 'seen'); setWelcomeOpen(false); };
  ...
  return <div className="min-h-screen bg-[#F6F9F8]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200/80 bg-white md:flex md:flex-col">
      ...
      <div className="border-t border-slate-100 p-4"><div className="mb-3 flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-ink text-sm font-bold text-white">{session?.user.name.slice(0, 1).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-slate-800">{session?.user.name}</p><p className="truncate text-xs text-slate-400">{session?.user.email}</p></div></div><div className="mb-2 flex gap-3 px-2 text-[11px]"><Link className="text-slate-400 hover:text-brand-700" to="/privacy">隱私權</Link><Link className="text-slate-400 hover:text-brand-700" to="/security">安全政策</Link></div><button className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm font-semibold text-slate-500 hover:bg-rose-50 hover:text-rose-600" onClick={logout}><LogOut size={17} />安全登出</button></div>
    </aside>
    ...
    {welcomeOpen && <Modal title="歡迎使用 ExcelMaster 智慧整合" onClose={closeWelcome} wide><div className="rounded-2xl bg-gradient-to-br from-brand-50 to-white p-5"><div className="flex items-start gap-4"><div className="rounded-2xl bg-brand-600 p-3 text-white"><Sparkles/></div><div><h3 className="text-xl font-black text-ink">上傳多份表格，直接產生新檔與專業報告</h3><p className="mt-2 text-sm leading-6 text-slate-500">不必先建立固定模板；系統會分析表頭位置、欄位語意與資料品質，再合併 Excel／Google Sheets。</p></div></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-slate-200 p-4"><FileSpreadsheet className="text-brand-600"/><p className="mt-3 font-bold">多檔合併與新檔輸出</p><p className="mt-1 text-sm leading-6 text-slate-500">Excel、CSV、ZIP 或公開 Google Sheets，一次放入後建立新的整合檔。</p></div><div className="rounded-2xl border border-slate-200 p-4"><BarChart3 className="text-brand-600"/><p className="mt-3 font-bold">專業 Excel／GAS 報告</p><p className="mt-1 text-sm leading-6 text-slate-500">KPI、資料品質、圖表與 Google Apps Script 報告一起輸出。</p></div><div className="rounded-2xl border border-slate-200 p-4"><Image className="text-brand-600"/><p className="mt-3 font-bold">圖片與 Logo</p><p className="mt-1 text-sm leading-6 text-slate-500">GAS 可辨識圖片網址建立預覽，也能從 HTTPS 網址加入封面或 Logo。</p></div><div className="rounded-2xl border border-slate-200 p-4"><KeyRound className="text-brand-600"/><p className="mt-3 font-bold">訂閱版安裝包</p><p className="mt-1 text-sm leading-6 text-slate-500">有效訂閱可下載 Windows／macOS 包；下載、啟用裝置與到期均由後台管理。</p></div></div><div className="mt-5 rounded-xl bg-slate-50 p-4 text-sm text-slate-600"><strong>您目前可用：{session?.workspace.plan}</strong><span className="ml-2">訂閱狀態 {session?.workspace.subscription.status}{session?.workspace.subscription.endsAt ? `，至 ${new Date(session.workspace.subscription.endsAt).toLocaleString('zh-TW')}` : ''}</span></div><div className="mt-5 flex flex-col gap-2 sm:flex-row"><Link className="btn-primary flex-1" to="/analysis" onClick={closeWelcome}><FileSpreadsheet size={17}/>開始整合表格</Link><Link className="btn-secondary flex-1" to="/billing" onClick={closeWelcome}><BadgeDollarSign size={17}/>查看方案能用什麼</Link></div></Modal>}
  </div>;
}
