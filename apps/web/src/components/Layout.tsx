...
import {
  Activity, Boxes, Braces, ChevronRight, CircleHelp, ClipboardCheck, Database, Download, FileClock, FileSpreadsheet,
  FolderKanban, LayoutDashboard, Library, LogOut, Menu, Network, Search, Settings, ShieldCheck, X
} from 'lucide-react';
...
const navItems = [
  { path: '/dashboard', label: '系統總覽', icon: LayoutDashboard },
  { path: '/projects', label: '整合專案', icon: FolderKanban },
  { path: '/analysis', label: '智慧分析', icon: FileSpreadsheet },
  { path: '/sources', label: '資料來源', icon: Database },
  { path: '/queue', label: '處理佇列', icon: Activity },
  { path: '/reviews', label: '待確認', icon: ClipboardCheck },
  { path: '/report-types', label: '報表分類', icon: Boxes },
  { path: '/schemas', label: '欄位模板', icon: Braces },
  { path: '/templates', label: '解析模板', icon: Library },
  { path: '/rules', label: '整合規則', icon: Network },
  { path: '/exports', label: '輸出紀錄', icon: FileClock },
  { path: '/audit', label: '操作紀錄', icon: ShieldCheck },
  { path: '/settings', label: '系統設定', icon: Settings }
];

const mobileItems = navItems.filter((item) => ['/dashboard', '/projects', '/analysis', '/reviews'].includes(item.path));
...
export function Layout() {
  ...
  const Navigation = ({ mobile = false }: { mobile?: boolean }) => <nav className="space-y-1.5">{navItems.map(({ path, label, icon: Icon }) => <NavLink key={path} to={path} onClick={() => mobile && setMobileOpen(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${isActive ? 'bg-brand-50 text-brand-700' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}><Icon size={18} /><span>{label}</span></NavLink>)}</nav>;
  ...
  return <div className="min-h-screen bg-[#F6F9F8]">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200/80 bg-white md:flex md:flex-col">
      ...
      <div className="mx-4 mb-4 rounded-xl border border-brand-100 bg-brand-50/70 p-3"><p className="truncate text-xs font-bold text-brand-900">{session?.workspace.name}</p><p className="mt-1 text-[11px] text-brand-700">{session?.workspace.role} 工作區</p></div>
    ...
    {mobileOpen && <div className="fixed inset-0 z-[60] md:hidden"><button className="absolute inset-0 bg-slate-950/40" onClick={() => setMobileOpen(false)} aria-label="關閉選單" /><div className="absolute inset-y-0 left-0 w-[84%] max-w-sm overflow-y-auto bg-white p-4 shadow-2xl"><div className="mb-5 flex items-center justify-between"><div className="flex items-center gap-3"><div className="rounded-xl bg-brand-600 p-2 text-white"><FileSpreadsheet /></div><span className="font-black text-ink">ExcelMaster</span></div><button onClick={() => setMobileOpen(false)} className="p-2"><X /></button></div><Navigation mobile /><a href="https://github.com/erin20080306/excelautomation/archive/refs/heads/codex/excelmaster-platform.zip" className="mt-6 flex w-full items-center gap-2 rounded-xl bg-ink px-3 py-2.5 text-sm font-bold text-white"><Download size={17} />下載完整版</a><button className="mt-3 flex w-full items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-500" onClick={logout}><LogOut size={17} />安全登出</button></div></div>}
