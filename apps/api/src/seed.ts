...
import { parseSeedConfig } from './lib/seed-config.js';
...
try {
  ...
  const result = await prisma.$transaction(async (tx) => {
    const existing = await tx.user.findUnique({ where: { email: config.SEED_ADMIN_EMAIL }, include: { memberships: { include: { workspace: true }, orderBy: { workspace: { createdAt: 'asc' } } } } });
    const user = existing
    ...
    if (existingMembership) {
      ...
      await tx.auditLog.create({ data: { workspaceId: existingMembership.workspaceId, userId: user.id, action: 'seed.superadmin.refresh', metadata: { source: 'environment' } } });
      return { user, workspace: existingMembership.workspace };
    }
    const workspace = await tx.workspace.create({ data: { name: config.SEED_WORKSPACE_NAME, plan: 'TRIAL', members: { create: { userId: user.id, role: 'OWNER' } } } });
    await bootstrapWorkspace(tx, workspace.id);
    await tx.auditLog.create({ data: { workspaceId: workspace.id, userId: user.id, action: 'seed.superadmin.create', metadata: { source: 'environment' } } });
